...
* __error:__  You can translate default errors messages. You juste have to add an options array when you init the plugin. __{{ value ***REMOVED******REMOVED***__ text will be replaced by the option.

```javascript
$('.dropify').dropify({
    error: {
        'fileSize': 'The file size is too big ({{ value ***REMOVED******REMOVED*** max).',
        'minWidth': 'The image width is too small ({{ value ***REMOVED******REMOVED******REMOVED***px min).',
        'maxWidth': 'The image width is too big ({{ value ***REMOVED******REMOVED******REMOVED***px max).',
        'minHeight': 'The image height is too small ({{ value ***REMOVED******REMOVED******REMOVED***px min).',
        'maxHeight': 'The image height is too big ({{ value ***REMOVED******REMOVED***px max).',
        'imageFormat': 'The image format is not allowed ({{ value ***REMOVED******REMOVED*** only).'
    ***REMOVED***
***REMOVED***
```
...
$('.dropify').dropify({
    tpl: {
        ...
        message:         '<div class="dropify-message"><span class="file-icon" /> <p>{{ default ***REMOVED******REMOVED***</p></div>',
        preview:         '<div class="dropify-preview"><span class="dropify-render"></span><div class="dropify-infos"><div class="dropify-infos-inner"><p class="dropify-infos-message">{{ replace ***REMOVED******REMOVED***</p></div></div></div>',
        filename:        '<p class="dropify-filename"><span class="file-icon"></span> <span class="dropify-filename-inner"></span></p>',
        clearButton:     '<button type="button" class="dropify-clear">{{ remove ***REMOVED******REMOVED***</button>',
        errorLine:       '<p class="dropify-error">{{ error ***REMOVED******REMOVED***</p>',
