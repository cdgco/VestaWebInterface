class Morris.Grid extends Morris.EventEmitter
  ...
  constructor: (options) ->
    ...
    @options = $.extend {***REMOVED***, @gridDefaults, (@defaults || {***REMOVED***), options
    ...
    @el.bind 'mousemove', (evt) =>
      ...
      if @selectFrom
        ...
        width = right - left
        @selectionRect.attr({ x: left, width: width ***REMOVED***)
      else
    ...
    if @options.rangeSelect
      @selectionRect = @raphael.rect(0, 0, 0, @el.innerHeight())
        .attr({ fill: @options.rangeSelectColor, stroke: false ***REMOVED***)
        .toBack()
        .hide()
  ...
  yboundary: (boundaryType, currentValue) ->
    boundaryOption = @options["y#{boundaryType***REMOVED***"]
  ...
  yLabelFormat: (label) ->
    if typeof @options.yLabelFormat is 'function'
      ...
      "#{@options.preUnits***REMOVED***#{Morris.commas(label)***REMOVED***#{@options.postUnits***REMOVED***"
  ...
  drawGrid: ->
    ...
    for lineY in @grid
      ...
      if @options.grid
        @drawGridLine("M#{@left***REMOVED***,#{y***REMOVED***H#{@left + @width***REMOVED***")
  ...
  drawGoal: (goal, color) ->
    @raphael.path("M#{@left***REMOVED***,#{@transY(goal)***REMOVED***H#{@right***REMOVED***")
      .attr('stroke', color)
      .attr('stroke-width', @options.goalStrokeWidth)

  drawEvent: (event, color) ->
    @raphael.path("M#{@transX(event)***REMOVED***,#{@bottom***REMOVED***V#{@top***REMOVED***")
      .attr('stroke', color)
  ...
  startRange: (x) ->
    @hover.hide()
    @selectFrom = x
    @selectionRect.attr({ x: x, width: 0 ***REMOVED***).show()
