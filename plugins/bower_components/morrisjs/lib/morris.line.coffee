class Morris.Line extends Morris.Grid
  ...
  hoverContentForRow: (index) ->
    row = @data[index]
    content = "<div class='morris-hover-row-label'>#{row.label***REMOVED***</div>"
    for y, j in row.y
      content += """
        <div class='morris-hover-point' style='color: #{@colorFor(row, j, 'label')***REMOVED***'>
          #{@options.labels[j]***REMOVED***:
          #{@yLabelFormat(y)***REMOVED***
        </div>
      """
  ...
  generatePaths: ->
    @paths = for i in [0...@options.ykeys.length]
      ...
      coords = ({x: r._x, y: r._y[i]***REMOVED*** for r in @data when r._y[i] isnt undefined)
  ...
  drawXAxis: ->
    ...
    drawLabel = (labelText, xpos) =>
      ...
      label.transform("r#{-@options.xLabelAngle***REMOVED***")
      labelBox = label.getBBox()
      label.transform("t0,#{labelBox.height / 2***REMOVED***...")
      if @options.xLabelAngle != 0
        offset = -0.5 * textBox.width *
          Math.cos(@options.xLabelAngle * Math.PI / 180.0)
        label.transform("t#{offset***REMOVED***,0...")
      ...
      if coord.y?
        if prevCoord.y?
          if smooth
            ...
            path += "C#{x1***REMOVED***,#{y1***REMOVED***,#{x2***REMOVED***,#{y2***REMOVED***,#{coord.x***REMOVED***,#{coord.y***REMOVED***"
          else
            path += "L#{coord.x***REMOVED***,#{coord.y***REMOVED***"
        else
          if not smooth or grads[i]?
            path += "M#{coord.x***REMOVED***,#{coord.y***REMOVED***"
      prevCoord = coord
    return path
  ...
  @gradients: (coords) ->
    ...
    for coord, i in coords
      if coord.y?
        nextCoord = coords[i + 1] or {y: null***REMOVED***
        prevCoord = coords[i - 1] or {y: null***REMOVED***
...
Morris.labelSeries = (dmin, dmax, pxwidth, specName, xLabelFormat) ->
  ...
  if xLabelFormat
    spec = $.extend({***REMOVED***, spec, {fmt: xLabelFormat***REMOVED***)
  # calculate labels
  d = spec.start(d0)
  ret = []
...
minutesSpecHelper = (interval) ->
  ...
  fmt: (d) -> "#{Morris.pad2(d.getHours())***REMOVED***:#{Morris.pad2(d.getMinutes())***REMOVED***"
...
secondsSpecHelper = (interval) ->
  ...
  fmt: (d) -> "#{Morris.pad2(d.getHours())***REMOVED***:#{Morris.pad2(d.getMinutes())***REMOVED***:#{Morris.pad2(d.getSeconds())***REMOVED***"
...
Morris.LABEL_SPECS =
  ...
  "month":
    ...
    fmt: (d) -> "#{d.getFullYear()***REMOVED***-#{Morris.pad2(d.getMonth() + 1)***REMOVED***"
    incr: (d) -> d.setMonth(d.getMonth() + 1)
  "week":
    span: 604800000 # 7 * 24 * 60 * 60 * 1000
    start: (d) -> new Date(d.getFullYear(), d.getMonth(), d.getDate())
    fmt: (d) -> "#{d.getFullYear()***REMOVED***-#{Morris.pad2(d.getMonth() + 1)***REMOVED***-#{Morris.pad2(d.getDate())***REMOVED***"
