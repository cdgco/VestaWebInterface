...
are numbered from 1 (default), so { yaxis: 2***REMOVED*** means that the series
...
```js
[ { label: "Foo", data: [ [10, 1], [17, -14], [30, 5] ] ***REMOVED***,
  { label: "Bar", data: [ [11, 13], [19, 11], [30, -7] ] ***REMOVED***
]
```
...
var options = {
    series: {
        lines: { show: true ***REMOVED***,
        points: { show: true ***REMOVED***
    ***REMOVED***
***REMOVED***;
...
```js
xaxis: {
    transform: function (v) { return Math.log(v); ***REMOVED***,
    inverseTransform: function (v) { return Math.exp(v); ***REMOVED***
***REMOVED***
```
...
```js
yaxis: {
    transform: function (v) { return -v; ***REMOVED***,
    inverseTransform: function (v) { return -v; ***REMOVED***
***REMOVED***
```
...
format of the data series, e.g. { data: [...], yaxis: 2 ***REMOVED*** specifies
...
```js
{
    xaxes: [ { position: "top" ***REMOVED*** ],
    yaxes: [ { ***REMOVED***, { position: "right", min: 20 ***REMOVED*** ]
***REMOVED***
```

The arrays get their default values from the xaxis/yaxis settings, so
say you want to have all y axes start at zero, you can simply specify
yaxis: { min: 0 ***REMOVED*** instead of adding a min parameter to all the axes.
...
The options inside "series: {***REMOVED***" are copied to each of the series. So
...
lines: { show: false ***REMOVED***). You can specify the various types
...
var options = {
    series: {
        lines: { show: true, fill: true, fillColor: "rgba(255, 255, 255, 0.8)" ***REMOVED***,
        points: { show: true, fill: false ***REMOVED***
    ***REMOVED***
***REMOVED***;
```
...
the form { xaxis: { from, to ***REMOVED***, yaxis: { from, to ***REMOVED*** ***REMOVED*** (with multiple
...
```js
markings: [ { xaxis: { from: 0, to: 2 ***REMOVED***, yaxis: { from: 10, to: 10 ***REMOVED***, color: "#bb0000" ***REMOVED***, ... ]
```

If you leave out one of the values, that value is assumed to go to the
border of the plot. So for example if you only specify { xaxis: {
from: 0, to: 2 ***REMOVED*** ***REMOVED*** it means an area that extends from the top to the
bottom of the plot in the x range 0-2.

A line is drawn if from and to are the same, e.g.

```js
markings: [ { yaxis: { from: 1, to: 1 ***REMOVED*** ***REMOVED***, ... ]
```
...
markings: function (axes) {
    ...
    for (var x = Math.floor(axes.xaxis.min); x < axes.xaxis.max; x += 2)
        markings.push({ xaxis: { from: x, to: x + 1 ***REMOVED*** ***REMOVED***);
    return markings;
***REMOVED***
```
...
```js
$.plot($("#placeholder"), [ d ], { grid: { clickable: true ***REMOVED*** ***REMOVED***);
...
```js
$.plot($("#placeholder"), [ { label: "Foo", data: [[0, 10], [7, 3]] ***REMOVED*** ], ...);
```
...
```js
{ data: [...], label: "Foo", clickable: false ***REMOVED***
```
...
```js
grid: {
    backgroundColor: { colors: ["#000", "#999"] ***REMOVED***
***REMOVED***
```
...
```js
{ colors: [{ opacity: 0.8 ***REMOVED***, { brightness: 0.6, opacity: 0.8 ***REMOVED*** ] ***REMOVED***
```
...
bars: {
    ...
    lineWidth: 0,
    fill: true,
    fillColor: { colors: [ { opacity: 0.8 ***REMOVED***, { opacity: 0.1 ***REMOVED*** ] ***REMOVED***
***REMOVED***
```
...
can call:
 ...
 - pointOffset({ x: xpos, y: ypos ***REMOVED***)
    ...
    ```js
      o = pointOffset({ x: xpos, y: ypos, xaxis: 2, yaxis: 3 ***REMOVED***)
...
```js
  ...
  function hellohook(plot, canvascontext) { alert("hello!"); ***REMOVED***;

  // pass it in, in an array since we might want to specify several
  var plot = $.plot(placeholder, data, { hooks: { draw: [hellohook] ***REMOVED*** ***REMOVED***);
