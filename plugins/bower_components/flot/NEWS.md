...
```js
{
    xaxis: { ... ***REMOVED***, x2axis: { ... ***REMOVED***,
    yaxis: { ... ***REMOVED***, y2axis: { ... ***REMOVED***
***REMOVED***
```
...
```js
{
    xaxes: [ { ... ***REMOVED***, { ... ***REMOVED*** ],
    yaxes: [ { ... ***REMOVED***, { ... ***REMOVED*** ]
***REMOVED***
```
...
```js
{ grid: { tickColor: "#000" ***REMOVED******REMOVED***
```

becomes

```js
{ xaxis: { tickColor: "#000"***REMOVED***, yaxis: { tickColor: "#000"***REMOVED*** ***REMOVED***
```
...
### Bug fixes ###
 ...
 - Fixed a problem introduced in 0.6 with specifying a gradient with:
   ...
   ```{brightness: x, opacity: y ***REMOVED***```
...
selection: { mode: something ***REMOVED***, you MUST include the file
...
```js
$.plot(placeholder, data, { lines: { show: true ***REMOVED******REMOVED***)
```

should be changed to

```js
  $.plot(placeholder, data, { series: { lines: { show: true ***REMOVED******REMOVED******REMOVED***)
```
...
## Changes ##
 ...
 - Crosshairs plugin: trace the mouse position on the axes, enable with
   crosshair: { mode: "x"***REMOVED*** (see the new tracking example for a use).
...
like ```{ xaxis: { from: 0, to: 10 ***REMOVED******REMOVED***```. Furthermore with markings you can
...
```{ xaxis: { from: -10, to: 20 ***REMOVED*** ... ***REMOVED***,``` setSelection in which the
...
So ```xaxis: { noTicks: 10 ***REMOVED***``` becomes ```xaxis: { ticks: 10 ***REMOVED***```.
...
of { x1, y1, x2, y2 ***REMOVED*** objects or a function that returns these given
{ xmin, xmax, ymin, ymax ***REMOVED***.
...
## Changes ##
 ...
 - Support clicking on the plot. Turn it on with grid: { clickable: true ***REMOVED***,
