...
promise
.done(function() { console.log('ready to go!'); ***REMOVED***)
.fail(function() { console.log('err, something went wrong :('); ***REMOVED***);
```
...
```javascript
engine.add([{ val: 'one' ***REMOVED***, { val: 'two' ***REMOVED***]);
```
...
```javascript
  var engine = new Bloodhound({
    local: [{ id: 1, name: 'dog' ***REMOVED***, { id: 2, name: 'pig' ***REMOVED***],
    identify: function(obj) { return obj.id; ***REMOVED***,
    queryTokenizer: Bloodhound.tokenizers.whitespace,
    datumTokenizer: Bloodhound.tokenizers.whitespace
  ***REMOVED***);

  engine.get([1, 3]); // [{ id: 1, name: 'dog' ***REMOVED***, null]
```
