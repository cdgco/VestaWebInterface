...
(function(factory) {
	...
	***REMOVED***
***REMOVED***)(function($, moment) {

;;

var fc = $.fullCalendar = { version: "2.3.2" ***REMOVED***;
var fcViews = fc.views = {***REMOVED***;
...
function massageOverrides(input) {
	var overrides = { views: input.views || {***REMOVED*** ***REMOVED***; // the output. ensure a `views` hash
	var subObj;
				...
				$.each(val, function(subName, subVal) {
					...
					if (/^(month|week|day|default|basic(Week|Day)?|agenda(Week|Day)?)$/.test(subName)) {
						if (!overrides.views[subName]) { // ensure the view-target entry exists
							overrides.views[subName] = {***REMOVED***;
						***REMOVED***
...
// Range can be a {start,end***REMOVED*** object, separate start/end args, or a Duration.
...
function computeRangeAs(unit, start, end) {
	...
	***REMOVED***
	else { // given { start, end ***REMOVED*** range object
...
function isTimeString(str) {
	return /^\d+\:\d+(?:\:\d+\.?(?:\d{3***REMOVED***)?)?$/.test(str);
***REMOVED***
...
var hasOwnPropMethod = {***REMOVED***.hasOwnProperty;
...
var ambigDateOfMonthRegex = /^\s*\d{4***REMOVED***-\d\d$/;
var ambigTimeOrZoneRegex =
	/^\s*\d{4***REMOVED***-(?:(\d\d-\d\d)|(W\d\d$)|(W\d\d-\d)|(\d\d\d))((T| )(\d\d(:\d\d(:\d\d(\.\d+)?)?)?)?)?$/;
var newMomentProto = moment.fn; // where we will attach our new methods
var oldMomentProto = $.extend({***REMOVED***, newMomentProto); // copy of original moment methods
...
var formatStringChunkCache = {***REMOVED***;
...
function chunkFormatString(formatStr) {
	...
	while ((match = chunker.exec(formatStr))) {
		...
		***REMOVED***
			...
			chunks.push({ maybe: chunkFormatString(match[2]) ***REMOVED***);
		***REMOVED***
		else if (match[3]) { // a formatting token
			chunks.push({ token: match[3] ***REMOVED***);
		***REMOVED***
...
var GridCoordMap = Class.extend({
	...
	rowCoords: null, // array of {top,bottom***REMOVED*** objects
	colCoords: null, // array of {left,right***REMOVED*** objects
...
var DragListener = fc.DragListener = Class.extend({
	...
	scrollEl: null,
	scrollBounds: null, // { top, bottom, left, right ***REMOVED***
...
var CellDragListener = DragListener.extend({
	...
	***REMOVED***,
	...
	listenStart: function(ev) {
		...
		if (ev) {
			origPoint = { left: ev.pageX, top: ev.pageY ***REMOVED***;
			point = origPoint;
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	***REMOVED***,
	...
	fabricateHelperEvent: function(range, sourceSeg) {
		var fakeEvent = sourceSeg ? createObject(sourceSeg.event) : {***REMOVED***; // mask the original event object if possible
	...
	***REMOVED***,
	...
	fillSegHtml: function(type, seg) {
		...
		var css = cssToStr(cssMethod ? cssMethod.call(this, seg) : {***REMOVED***);
...
Grid.mixin({
	...
	***REMOVED***,
	...
	bgEventSegClasses: function(seg) {
		var event = seg.event;
		var source = event.source || {***REMOVED***;
	...
	***REMOVED***,
	...
	triggerSegMouseout: function(seg, ev) {
		ev = ev || {***REMOVED***; // if given no args, make a mock mouse event
	...
	***REMOVED***,
	...
	segDragStart: function(seg, ev) {
		...
		this.view.trigger('eventDragStart', seg.el[0], seg.event, ev, {***REMOVED***); // last argument is jqui dummy
	***REMOVED***,
	...
	segDragStop: function(seg, ev) {
		...
		this.view.trigger('eventDragStop', seg.el[0], seg.event, ev, {***REMOVED***); // last argument is jqui dummy
	***REMOVED***,
	...
	***REMOVED***,
	...
	segResizeStart: function(seg, ev) {
		...
		this.view.trigger('eventResizeStart', seg.el[0], seg.event, ev, {***REMOVED***); // last argument is jqui dummy
	***REMOVED***,
	...
	segResizeStop: function(seg, ev) {
		...
		this.view.trigger('eventResizeStop', seg.el[0], seg.event, ev, {***REMOVED***); // last argument is jqui dummy
	***REMOVED***,
...
function getEventRendering(event) {
	return firstDefined((event.source || {***REMOVED***).rendering, event.rendering);
***REMOVED***


function groupEventsById(events) {
	var eventsById = {***REMOVED***;
	var i, event;
...
function getDraggedElMeta(el) {
	...
	if (eventProps) {
		if (typeof eventProps === 'object') {
			eventProps = $.extend({***REMOVED***, eventProps); // make a copy
		***REMOVED***
		...
		***REMOVED***
		...
		if (startTime == null) { startTime = eventProps.time; ***REMOVED*** // accept 'time' as well
	...
	***REMOVED***
	...
	if (startTime == null) { startTime = el.data(prefix + 'start'); ***REMOVED***
	if (startTime == null) { startTime = el.data(prefix + 'time'); ***REMOVED*** // accept 'time' as well
	if (duration == null) { duration = el.data(prefix + 'duration'); ***REMOVED***
	if (stick == null) { stick = el.data(prefix + 'stick'); ***REMOVED***
	...
	return { eventProps: eventProps, startTime: startTime, duration: duration, stick: stick ***REMOVED***;
***REMOVED***
	...
	rangeToSegs: function(range) {
		...
		for (row = 0; row < rowCnt; row++) {
			...
			if (segFirst <= segLast) { // was there any intersection with the current row?
				...
				seg = { row: row, isStart: isStart, isEnd: isEnd ***REMOVED***;
				if (isRTL) {
...
var View = fc.View = Class.extend({
	...
	***REMOVED***,
	...
	computeTitle: function() {
		return this.formatRange(
			{ start: this.intervalStart, end: this.intervalEnd ***REMOVED***,
	...
	***REMOVED***,
	...
	computeScrollerHeight: function(totalHeight) {
		...
		***REMOVED***);
		...
		both.css({ position: '', left: '' ***REMOVED***); // undo hack
	...
	***REMOVED***,
	...
	triggerEventDrop: function(event, dateDelta, undoFunc, el, ev) {
		this.trigger('eventDrop', el[0], event, dateDelta, undoFunc, ev, {***REMOVED***); // {***REMOVED*** = jqui dummy
	***REMOVED***,
	...
	reportExternalDrop: function(meta, dropLocation, el, ev, ui) {
		...
		if (eventProps) {
			eventInput = $.extend({***REMOVED***, eventProps, dropLocation);
	...
	***REMOVED***,
	...
	triggerEventResize: function(event, durationDelta, undoFunc, el, ev) {
		this.trigger('eventResize', el[0], event, durationDelta, undoFunc, ev, {***REMOVED***); // {***REMOVED*** = jqui dummy
	***REMOVED***,
...
var Calendar = fc.Calendar = Class.extend({
	...
	***REMOVED***,
	...
	initOptions: function(overrides) {
		...
		if (!langDefaults) {
			...
			langDefaults = langOptionHash[lang] || {***REMOVED***;
		***REMOVED***
		...
		dirDefaults = isRTL ? Calendar.rtlDefaults : {***REMOVED***;
		...
		this.viewSpecCache = {***REMOVED***; // somewhat unrelated
	***REMOVED***,
	...
	***REMOVED***,
	...
	buildViewSpec: function(requestedViewType) {
		var viewOverrides = this.overrides.views || {***REMOVED***;
		...
		while (viewType) {
			...
			if (typeof spec === 'function') { // TODO: deprecate
				spec = { 'class': spec ***REMOVED***;
			***REMOVED***

			if (spec) {
				specChain.unshift(spec);
				defaultsChain.unshift(spec.defaults || {***REMOVED***);
		...
		if (duration) {
			...
			if (duration.valueOf()) { // valid?
				...
				if (duration.as(unit) === 1) {
					...
					overridesChain.unshift(viewOverrides[unit] || {***REMOVED***);
				***REMOVED***
			***REMOVED***
		***REMOVED***
	...
	***REMOVED***,
	...
	buildViewSpecButtonText: function(spec, requestedViewType) {
		...
		function queryButtonText(options) {
			var buttonText = options.buttonText || {***REMOVED***;
...
function Calendar_constructor(element, overrides) {
	...
	t.initOptions(overrides || {***REMOVED***);
	var options = this.options;
	...
	***REMOVED***
	if (options.firstDay != null) {
		var _week = createObject(localeData._week); // _week: { dow: # ***REMOVED***
	...
	***REMOVED***
	...
	localeData._fullCalendar_weekCalc = (function(weekCalc) {
		...
		***REMOVED***
			...
			return 'ISO';
		***REMOVED***
	***REMOVED***)(options.weekNumberCalculation);
	...
	***REMOVED***;
	...
	var viewsByType = {***REMOVED***; // holds all instantiated view instances, current or not
...
Calendar.defaults = {
	...
	defaultAllDayEventDuration: { days: 1 ***REMOVED***,
...
var langOptionHash = fc.langs = {***REMOVED***; // initialize and expose
...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	var fcOptions = langOptionHash[langCode] || (langOptionHash[langCode] = {***REMOVED***);
...
fc.lang = function(langCode, newFcOptions) {
	...
	// get the FullCalendar internal option hash for this language. create if necessary
	fcOptions = langOptionHash[langCode] || (langOptionHash[langCode] = {***REMOVED***);
					...
					***REMOVED***
						...
						***REMOVED***
							...
							buttonClick = function() {
								calendar[buttonName]();
							***REMOVED***;
							overrideText = (calendar.overrides.buttonText || {***REMOVED***)[buttonName];
...
function EventManager(options) { // assumed to be a calendar
	...
	// locals
	var stickySource = { events: [] ***REMOVED***;
		...
		***REMOVED******REMOVED***
			...
			if (url) {
				...
				***REMOVED***
				...
				var data = $.extend({***REMOVED***, customData || {***REMOVED***);
				...
				***REMOVED***
				...
				t.pushLoading();
				$.ajax($.extend({***REMOVED***, ajaxDefaults, source, {
					data: data,
	...
	***REMOVED***
	...
	function buildEventSource(sourceInput) { // will return undefined if invalid source
		...
		if ($.isFunction(sourceInput) || $.isArray(sourceInput)) {
			source = { events: sourceInput ***REMOVED***;
		***REMOVED***
		else if (typeof sourceInput === 'string') {
			source = { url: sourceInput ***REMOVED***;
		***REMOVED***
		else if (typeof sourceInput === 'object') {
			source = $.extend({***REMOVED***, sourceInput); // shallow copy
		***REMOVED***
	...
	***REMOVED***
	...
	function removeEvents(filter) {
		...
		if (filter == null) { // null or undefined. remove all events
			filter = function() { return true; ***REMOVED***; // will always match
		***REMOVED***
	...
	function ensureVisibleEventRange(range) {
		...
		if (!range.end) {
			...
			***REMOVED***
			...
			range = $.extend({***REMOVED***, range); // make a copy, copying over other misc properties
				...
				while (date.isBefore(_rangeEnd)) {
					...
					if (!dowHash || dowHash[date.day()]) { // if everyday, or this particular day-of-week
						...
						***REMOVED***
						...
						event = $.extend({***REMOVED***, abstractEvent); // make a copy of the original
						assignDatesToEvent(
							start, end,
	...
	***REMOVED***
	...
	function mutateEvents(events, clearEnd, allDay, dateDelta, durationDelta, miscProps) {
		...
		if (dateDelta && !dateDelta.valueOf()) { dateDelta = null; ***REMOVED***
		if (durationDelta && !durationDelta.valueOf()) { durationDelta = null; ***REMOVED***
	...
	function getBusinessHoursEvents(wholeDay) {
		...
		***REMOVED***;
		...
		if (optionVal) { // `true` (which means "use the defaults") or an override object
			eventInput = $.extend(
				{***REMOVED***, // copy to a new object in either case
				defaultVal,
				typeof optionVal === 'object' ? optionVal : {***REMOVED*** // override the defaults
			);
		***REMOVED***
	...
	***REMOVED***
	...
	function isExternalDropRangeAllowed(range, eventProps) {
		...
		if (eventProps) {
			eventInput = $.extend({***REMOVED***, eventProps, range);
	...
	***REMOVED***
	...
	function isRangeAllowed(range, constraint, overlap, event) {
		...
		range = $.extend({***REMOVED***, range); // copy all properties in case there are misc non-date properties
				...
				***REMOVED***
				...
				if (event) {
					peerOverlap = firstDefined(
						peerEvent.overlap,
						(peerEvent.source || {***REMOVED***).overlap
...
fcViews.month = {
	'class': MonthView,
	duration: { months: 1 ***REMOVED***, // important for prev/next
	defaults: {
		fixedWeekCount: true
	***REMOVED***
***REMOVED***;
