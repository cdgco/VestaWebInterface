...
```js
Holder.addTheme("thumbnail", { background: "#fff", text: "Thumbnail" ***REMOVED***);
```
...
```js
Holder.addTheme("bright", {background: "white", foreground: "gray", size: 12***REMOVED***).run();
```
...
```css
#sample {background:url(?holder.js/200x200/social) no-repeat***REMOVED***
```
...
```js
Holder.run({domain:"example.com"***REMOVED***);
```
...
Holder.addTheme("new", {
  ...
  background: "#000",
  size: 10
***REMOVED***).addImage("holder.js/200x100/new", "body").run();
```
...
```js
Holder.run({renderer: 'canvas'***REMOVED***);
```

Using with [lazyload.js](https://github.com/tuupola/jquery_lazyload)
------------------------

Holder is compatible with ``lazyload.js`` and works with both fluid and fixed-width images. For best results, run `.lazyload({skip_invisible:false***REMOVED***)`.
...
angular.module('MyModule').directive('myHolder', function() {
  return {
    link: function(scope, element, attrs) {
      ...
      Holder.run({images:element[0]***REMOVED***);
    ***REMOVED***
  ***REMOVED***;
***REMOVED***);
```
